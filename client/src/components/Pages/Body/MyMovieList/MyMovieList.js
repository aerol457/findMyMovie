...
class MyMoviesList extends Component {
      ...
      .then((dataMovies) => {
        for (let movie of dataMovies.data) {
          this.setState({
            ...
            movies: {
              ...this.state.movies,
              [movie.category]: [...this.state.movies[movie.category], movie],
            },
          });
        }
        this.setState({ loading: false });
  ...
  removeMovieFromAccountList = (id, category) => {
    ...
    const token = localStorage.getItem("token");
    fetch(
      "http://localhost:8080/account/remove-movie?idUser= " +
        idUser +
        "&idMovie=" +
        id,
      {
      ...
      .then((resData) => {
        ...
        const updateMoviesList = this.state.movies[category].filter(
          (c) => c.id !== id
        );
        this.setState({
          movies: {
    ...
    if (!this.state.loading) {
      content = (
        <div>
          {quantityOfMovies !== 0 ? (
            <div>
              {this.state.movies.comedy.length !== 0 ? (
                <AddList
                  clickedToViewDetails={() => {}}
                  title="Comedy"
                  sign="-"
                  removeMovie={(id) =>
                    this.removeMovieFromAccountList(id, "comedy")
                  }
                  movieList={this.state.movies.comedy}
                />
              ) : null}
              {this.state.movies.action.length !== 0 ? (
                <AddList
                  title="Action"
                  sign="-"
                  removeMovie={(id) =>
                    this.removeMovieFromAccountList(id, "action")
                  }
                  movieList={this.state.movies.action}
                />
              ) : null}
              {this.state.movies.horror.length !== 0 ? (
                <AddList
                  title="Horror"
                  sign="-"
                  removeMovie={(id) =>
                    this.removeMovieFromAccountList(id, "horror")
                  }
            ...
            <h2 style={{ color: "gray", marginTop: 130, fontSize: 30 }}>
              Your list movie is empty, Start adding movies.
            </h2>
          )}
        </div>
      );
